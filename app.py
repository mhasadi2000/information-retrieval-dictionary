...
def createPossitionalIndex() :
    for docId in data :
        ...
        for position,word in enumerate(word_tokenize(normalized)):
            if word not in stopwords_list and len(word) != 1 and re.match('[!"#$%&\'()*+,-./:;<=>?@[\\]^_`،{|}~]|[0-9۰-۹]*',word).span()==(0,0) :
                word = Stemmer().stem(word)
                
                # adding elements to dictionary and postings lists
                if word in possitional_index:
                            
                    # Check if the word has existed in that DocID before.
                ...
                # If word does not exist in the positional index dictionary
                else:
